...
def train_fusion(project_root: str, models_dir: str, pt_device: torch.device) -> None:
    ...
    # --- Save the slot_map so runtime uses EXACT same device→slot mapping ---
    # Build slot_map from the dataset building step (same logic as build_dataset)
    device_histories = build_device_history(project_root)
    dev_encoder_ids = []
    for dev_id, (dtype, _) in device_histories.items():
        if dtype in DEVICE_TYPES:
            dev_encoder_ids.append(dev_id)
    sorted_ids = sorted(dev_encoder_ids)
    slot_map = {dev_id: min(i, N_DEVICE_SLOTS - 1)
                for i, dev_id in enumerate(sorted_ids)}

    slot_map_path = os.path.join(models_dir, "patient_fusion_slot_map.json")
    # JSON requires string keys; store as {str(dev_id): slot}
    with open(slot_map_path, "w", encoding="utf-8") as fh:
        json.dump(
            {"slot_map": {str(k): v for k, v in slot_map.items()},
             "n_device_slots": N_DEVICE_SLOTS,
             "note": "Maps device_id → slot index. Generated by train_patient_fusion.py."},
            fh, indent=2,
        )
    print(f"[SAVE] {slot_map_path}  (slot_map={slot_map})")
